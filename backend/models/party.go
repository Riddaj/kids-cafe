...
type Party struct {
	...
	DepositFilename string `json:"deposit_filename" firestore:"deposit_filename"`
	//DepositImageURL string `json:"deposit_image_url" firestore:"deposit_image_url"`
}
...
func GetParty(c *gin.Context) ([]Party, error) {
	...
	for {
		...
		//	party.deposit_filename = url
		//}

		// ✅ 이미지 파일명만 저장
		if filename, ok := doc.Data()["deposit_filename"].(string); ok {
			party.DepositFilename = filename
		}
